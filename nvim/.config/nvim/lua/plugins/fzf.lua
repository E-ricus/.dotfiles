return {
  ...
  config = function() -- calling `setup` is optional for customization
    ...
    map("n", "<leader>fw", fzf.live_grep_native, { noremap = true, desc = "Live grep word" })
    map("n", "<leader>fc", fzf.grep_cword, { noremap = true, desc = "Live grep word" })
    map("n", "<leader>fW", fzf.grep_project, { noremap = true, desc = "Search word on all files" })
